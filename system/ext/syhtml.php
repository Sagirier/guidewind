...
class syhtml
{
	...
	public function make($spurl, $alias_url = null, $todir = 1)
	{
		...
		@list($geturl, $controller, $action, $args, $anchor) = $spurl;
			if($todir!=0){
				$file_root_name = ( '' == $GLOBALS['WP']['ext']['site_statichtml_dir'] ) ? '' : $GLOBALS['WP']['ext']['site_statichtml_dir'].'/';
			}
			if( null == $alias_url ){
			...
			$baseuri = rtrim(dirname($GLOBALS['WP']['url']["url_path_base"]), '/\\')."/".$filedir.$filename;
		...
		if( $url_list = syAccess('r', 'sp_url_list') ){
			...
			foreach( $url_list as $url ){
				if( substr($url,0,strlen($url_input)) == $url_input ){
					...
					if( TRUE == $GLOBALS['WP']['html']['safe_check_file_exists'] && TRUE != $force_no_check ){
	...
	public function c_content($cmark,$date,$file){
		$this->make(array('index.php',$cmark,'index', $date),$file,0);
	}
