...
class syController { 
	...
	{	
		if(TRUE == $GLOBALS['WP']['view']['enabled']){
			$this->v = syClass('syView');
		}
		$this->cmark=syClass("c_channels")->findAll(array('`statu`'=>1),' `order` desc,`cid` ','`cid`,`cmark`,`cname`,`sys`');
		$this->userClass=syClass("syauser");
		$this->user=syClass("syauser")->info();
	}

	public function __set($name, $value)
	{
		if(TRUE == $GLOBALS['WP']['view']['enabled'] && false !== $value){
	...
	{
		if( TRUE != $this->v->displayed && FALSE != $GLOBALS['WP']['view']['auto_display']){
	...
	{
		if(in_array($name, $GLOBALS['WP']["auto_load_controller"])){
