...
if(!defined('APP_PATH')||!defined('WIND_PATH')){exit('Access Denied');}
	class syModel{
		...
		public function __construct(){
			if( null == $this->tbl_name )$this->tbl_name = $GLOBALS['WP']['db']['prefix'] . $this->table;
			if( '' == $GLOBALS['WP']['db_driver_path'] ){
				$GLOBALS['WP']['db_driver_path'] = $GLOBALS['WP']['sp_drivers_path'].'/'.$GLOBALS['WP']['db']['driver'].'.php';
			}
			$this->_db = syClass('db_'.$GLOBALS['WP']['db']['driver'], array(0=>$GLOBALS['WP']['db']), $GLOBALS['WP']['db_driver_path']);
		}
		...
		{
			if(in_array($name, $GLOBALS['WP']["auto_load_model"])){
