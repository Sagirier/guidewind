...
class product extends syController
{
	...
	function index(){
        ...
        $f_fields = array();
        $f=syDB('fields')->findAll(" cmark='product' and navigators like '%|".$this->product['nid']."|%' and lists=1 ");
        if( $f )foreach($f as $fv){$f_fields[]=$fv['fmark'];}
        $fields = array();
        foreach ($f_fields as $f_field){
            if(isset($this->product[$f_field])) {
                $fields[$f_field] = $this->product[$f_field];
            }
        }
        $this->product=array_merge($this->product,array('fields'=>$fields));
...
//				$sql='select *'.$fields.' from '.$this->db.' a left join '.$this->db.'_field b on (a.id=b.aid)'.$w.$order;
//			}else{
//				$sql='select * from '.$this->db.$w.$order;
//			}
            $sql='select a.*,b.detail from '.$this->db.' a left join '.$this->db.'_field b on (a.id=b.aid)'.$w.$order;
			...
			foreach($list_c as $k=>$v){
                $f_fields = array();
                $f=syDB('fields')->findAll(" cmark='product' and navigators like '%|".$v['nid']."|%' and lists=1 ");
                if( $f )foreach($f as $fv){$f_fields[]=$fv['fmark'];}
                ...
                $p_fields = syDB("product_field")->find(array('aid'=>$v['id']));
				foreach ($f_fields as $f_field){
                    if(isset($p_fields[$f_field])) $list_c[$k]['fields'][$f_field] = $p_fields[$f_field];
                }
